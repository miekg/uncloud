...
func NewRemoveCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long:    "Remove an image. You cannot remove an image that is in use by a container.",
	...
	cmd.Flags().BoolVarP(&opts.force, "force", "f", false,
		"Force the removal of the image.")
	cmd.Flags().StringSliceVarP(&opts.machines, "machine", "m", nil,
		"Name or ID of the machine to remove image from. "+
			"Can be specified multiple times or as a comma-separated list.\n"+
			"If not specified, the found image will be removed from all machines.")
	cmd.Flags().BoolVarP(&opts.yes, "yes", "y", false,
		"Do not prompt for confirmation before removing the image.")
...
func remove(ctx context.Context, uncli *cli.CLI, name string, opts removeOptions) error {
	...
	if !opts.yes {
		fmt.Println("The following image will be removed:")
